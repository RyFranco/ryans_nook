...
export default function ReviewEditModal({ review, onClose }) {
  ...
  const handleDelete = async() =>{
    try{
      const reponse = await fetch("http://localhost:8080/reviews/delete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(editedReview),
      });
    ...
    <div className="edit-modal-content" onClick={(e) => e.stopPropagation()}>
      ...
      <div className='edit-review-details'>
        <form onSubmit={handleSubmit}>
          ...
          <div className="edit-input-row">
            <label>
              ...
              <select name="status" value={editedReview.status} onChange={(e) => {
                ...
                <option value="watching">{editedReview.media_type === "game" ? `Playing` : `Watching`}</option>
            ...
            {!["movie", "game", "music"].includes(editedReview.media_type) && (
              <label>
              Progress:
          ...
          <button className='edit-review-delete-button' type = "submit" onClick={handleDelete}>Delete</button>
          <button className='edit-review-delete-button' type = "button" onClick={() => setAdvanceView(!advanceView) } >Advance⌄ </button>
